...
![CogMem 架构图](assets/architecture.svg)
...
![消融实验结果](assets/ablation-results.svg)
...
| Qwen-Max | DashScope | 78.4% | 243/310* | 幻觉（41.9% 编造） |

*Qwen-Max 因 API 限流仅完成 13/70 组对话。

![多 LLM 后端对比](assets/multi-llm-comparison.svg)

**关键洞察**：模型的"诚实校准"（愿意承认"我没有该记录"）与准确率的相关性比模型等级更强。免费模型（DS-V4-Flash，80.7%）可以超越付费旗舰模型（Qwen-Max，78.4%）。
...
![错误模式分析](assets/error-modes.svg)

*†Qwen-Max 错误分类来自早期分析运行；总错误数已按 310−243=67 重新计算，"未记录"和"编造"数量之和可能不等于 67。*
...
| CogMem (bge-small-en) | 155 分钟 | 13 分钟 | 2.88M | 0.57M | 35,796 | 83.5 MB |
...
├── assets/
│   ├── architecture.svg         # 三路径混合检索架构图
│   ├── ablation-results.svg     # 消融实验柱状图
│   ├── multi-llm-comparison.svg # 多 LLM 后端对比图
│   └── error-modes.svg           # 错误模式分析图
├── examples/
